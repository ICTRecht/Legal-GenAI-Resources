# 🛠️ The Vibe Coding Handbook

Welcome to **The Vibe Coding Handbook** — a practical guide for building real software by describing what you want, not by typing every line yourself.

This guide teaches you the craft of *vibe coding*: directing AI coding agents to build small, focused tools that solve real problems. The principles are tool-agnostic. The examples cover the five tools most people are choosing from in 2026: **Claude Code, Cursor, Lovable, Google Antigravity, and OpenAI Codex**.

> **Brand new to all this?** If words like *terminal*, *CLI*, *IDE*, *repo* feel like noise, skim the [Glossary](#-glossary-terms-and-acronyms) first or read sections 1–3 with it open in another tab. Section 2 in particular gives you the foundation everything else builds on.

---

## 🎯 Who is this guide for?

Anyone who can clearly describe what they want, but doesn't (or no longer wants to) type every line of code themselves:

- **Founders** who want to prototype quickly
- **Designers** who want to build real interactivity
- **Lawyers** who want to automate their own workflows
- **Researchers** wrangling data
- **Engineers** who want more leverage
- Anyone who's been told "*you'd need a developer for that*" once too often

What this **isn't**: a tutorial on a specific framework or programming language. A list of ready-to-use prompts. A product comparison. The patterns here outlive specific tools — they'll still hold up when something new shows up next year.
...
19. [Glossary: terms and acronyms](#-glossary-terms-and-acronyms)
...
What hasn't changed: **the people getting good results have specific habits.** The ones getting bad results — endless loops, broken builds, half-working prototypes — are usually missing the same handful of fundamentals.
...
| --- | --- |
| `.md` | Markdown — plain text with light formatting. Used for docs and instructions. |
...
The best mental model: **one folder for each separate tool, app, or project you're working on.** Don't pile everything into one giant folder. New project, new folder.

Inside a typical folder you'll find:

1. **Inputs** — the data you want to process, or the source material you're working with.
2. **Reference material** — your style guide, your playbook, your standards. The "rules" the agent should follow.
3. **Configuration** — instructions for the agent (a `CLAUDE.md` file or equivalent), plus settings.
4. **The actual code or app** — what the agent generates and edits.
5. **Outputs** — what gets produced when you run the thing.

This isn't rigid — organise however you like. But this rough split (inputs, reference, code, outputs) shows up in almost every project.
...
This also means: **if the agent needs to know about something, it has to be in the folder.** Things that aren't there don't exist, from the agent's perspective.

---

## 3. The mental model: it's not a chatbot

This is the single biggest shift to internalise before picking a tool.

ChatGPT-style chat interfaces are **conversational**: you ask, you get an answer, you ask again. Each turn is largely independent. The interface *is* the experience.

AI coding agents are **agentic and folder-scoped**. The folder is the experience. The agent reads files, writes files, runs commands. You direct the work, but the agent does it — sometimes for minutes at a time, sometimes touching dozens of files.

Three things follow:

1. **Context is finite and degrades.** Every file the agent reads, every command it runs, every piece of conversation, fills up its working memory (the *context window*). As that fills, performance drops. Most best practices in this guide exist to manage this one constraint.

2. **Structure beats chat.** A two-line instruction in a project-level config file does more for output quality than a paragraph of context pasted into every prompt. Persistent context beats per-conversation context.

3. **You stay responsible.** The agent will produce confident-looking work that's wrong in subtle ways. For anything that matters, verification isn't optional. The habits in [section 14](#14-verification-how-to-trust-what-you-didnt-write) are not skippable.
...
**What it is.** Anthropic's AI coding agent, available in two equally first-class forms: a **desktop app** (Mac and Windows; redesigned April 2026 into a full IDE-like environment with parallel sessions, file editor, diff viewer, and live preview) and a **CLI** (terminal-native, scriptable, runs anywhere including Linux). Both share the same engine and the same `CLAUDE.md` configuration. Use whichever fits the task.

**Best for.** The default choice if you have a Claude subscription. The desktop app is genuinely beginner-friendly. The CLI is what you reach for once you want scripting, automation, or remote workflows. Strong at long, multi-step tasks and at running multiple parallel sessions across projects.
...
**Notable.** Knowledge base tab where you persist project context. One-click deployment. Strong on UI generation; weaker on complex backend logic. The fastest path from idea to running web app.
...
**Best for.** Anyone with even a little code-editor experience. Excellent for editing existing codebases, multi-file refactors, and staying close to the code while the AI helps.
...
**Best for.** Builders who want to experiment with parallel agents and don't mind a younger product. Generous free preview, very long context window, built-in browser for end-to-end testing.

**Notable.** Configuration via `AGENTS.md` files. Native browser controls for testing web apps. Mission Control for managing parallel work. No MCP support yet at the time of writing.
...
**Best for.** People already paying for ChatGPT who want a terminal/cloud agent without another subscription. Strong at long-running cloud tasks. GPT-5 reasoning under the hood.

**Notable.** Cloud execution mode: tasks run on OpenAI's infrastructure, not your machine. Configuration via `AGENTS.md`. Sandboxed by default. Tight GitHub integration with automatic pull-request creation.
...
- 🗂️ **A personal dashboard** that pulls in data you care about — calendar, emails, tasks, the weather — and shows it the way *you* want to see it.
- 🛠️ **An internal tool for your team.** Intake form, reviewing process, output document. Replaces the messy spreadsheet everyone hates.
...
- 🌐 **A landing page with real interactivity.** Embedded calculators, interactive demos, calendar bookings, lightweight forms-with-logic.
...
- 💬 **A chatbot grounded in your own data.** The "ChatGPT for your company docs" pattern, scaled down to "ChatGPT for *my* docs."
...
| Platform | How |
| --- | --- |
| **Mac** | Press `Cmd+Space`, type "Terminal", press Enter. A window opens with a blinking cursor. |
| **Windows** | Press the Windows key, type "Terminal" or "PowerShell", press Enter. |
| **Inside VS Code or Cursor** | `View → Terminal` (or `` Ctrl/Cmd+` ``). Opens a panel inside your editor. |

To navigate to a folder, type `cd` followed by the folder path. To see what's in the current folder, type `ls` (Mac/Linux) or `dir` (Windows). That's all the terminal vocabulary you need to start.

### Lovable — no install

Go to [lovable.dev](https://lovable.dev), sign in, click *"New project."* Describe what you want in plain language. Lovable builds it in front of you. Use the **Knowledge tab** (in project settings) to add persistent project context. Connect a Supabase project for the backend; connect a GitHub repo if you want the code under version control. **No local install. No terminal. The lowest-friction path of the five.**
...
Open a terminal in your project folder and run `claude`. Sign in. Run `/init` to generate a starter `CLAUDE.md`. Run `/permissions` to decide which actions Claude can take without asking.

The two share configuration — `CLAUDE.md` files, skills, hooks, and MCP servers all work identically across both.
...
> ⚠️ **Don't have a Claude Pro or Max subscription?** Claude Code Desktop needs one. If you'd rather try something free first, jump to the [Lovable alternative](#alternative-lovable-no-install-needed) at the end of this section — same idea, different tool, no install required.
...
**Step 1: Install.** Go to [claude.com/product/claude-code](https://claude.com/product/claude-code) and download the desktop app for your platform. On Windows, you'll also need [Git for Windows](https://git-scm.com/download/win) installed. Mac comes with git already.
...
In the desktop app, click *"New session"* and point it at the folder you just made. The app now treats that folder as your project workspace.
...
**Step 4: Approve and let it build.** When the plan looks good, just say *"Looks good, please build it."* Files appear in the file tree on the left. The app shows you a diff for each file before it commits the change — you can accept, reject, or edit anything inline.
...
**Step 7: Set up persistent instructions.** Now the high-leverage move. Create a new file called `CLAUDE.md` at the root of your project folder. Paste:
...
Save the file. **From now on, every prompt in this project respects these rules** — Claude reads `CLAUDE.md` at the start of every session.
...
A good agent should refuse or ask clarifying questions — there's no clean way to do that, copyright is a mess, and the prompt is vague. **This is your chance to get a feel for the tool's failure modes early, in a low-stakes setting.**

**Step 9: Commit your work.** Either ask Claude *"Please commit everything we've done so far with a sensible commit message,"* or use the desktop app's git panel. You now have a saved snapshot you can roll back to.
...
Once the reading tracker works, the same shape lets you build many more useful things. Some good next steps:
...
2. **Defaults.** Working language, style, framework, output format.
3. **Things to never do.** Specific traps you've hit. (*"Never use a paid API. Never commit `.env` files. Never invent function names you haven't verified exist."*)
4. **Workflow conventions.** (*"Run tests before suggesting a commit. Commit after each logical change."*)
5. **Pointers, not content.** Reference other files rather than copying their contents.
...
In **Lovable** specifically, the equivalent lives in the Knowledge tab rather than a file in your repo — same idea, different mechanism. Add the same kind of content there.
...
1. **Explore.** Get the agent oriented before letting it change anything. Most tools have a read-only mode for this:
...
2. **Plan.** Still read-only. Ask for a written plan. *"What files need to change? What's the order? Any new dependencies? Edge cases I should think about?"* Read the plan. Edit it if needed. **The plan is cheap to change at this stage; the code isn't.**

3. **Build.** Switch the agent to write mode. Tell it to follow the plan. *"After each step, briefly tell me what you did. If you hit something not covered by the plan, stop and ask."* Watch the file changes go by. Don't be afraid to interrupt — every tool here lets you stop the agent mid-task without losing context.

4. **Verify.** Run it. Look at the output. Did it work? Did it do what you asked? Did it accidentally also do five things you didn't ask for? Verification is in [section 14](#14-verification-how-to-trust-what-you-didnt-write) in detail.

> 💡 **When to skip the plan.** If you can describe the change in one sentence (*"rename this variable", "add a log line", "fix this typo"*), just ask. Plan Mode adds overhead and you don't need it for trivial changes. **Rule of thumb:** if you could describe the diff in a single sentence, skip the plan.
...
Vague prompts work fine when you're exploring and can afford to course-correct. But for any prompt where you have a specific outcome in mind, **name it**.
...
> 💡 **A note on "tests."** Several patterns here mention tests. A *test* is just a small piece of code that automatically checks whether your main code does what it should. The agent can write these for you; when you "run the tests," a script tries them all and tells you which pass and which fail. **You don't need to know how to write tests yourself** — just know that asking for them is one of the strongest ways to catch problems early.
...
1. **Reset between unrelated tasks.** Finished one feature and now switching to a different one? Start fresh. In Claude Code: `/clear`. In Cursor: open a new chat. In Lovable: start a new conversation thread. **Don't carry yesterday's context into today's job.**

2. **Use sub-tasks or sub-agents for investigation.** When you ask the agent to *"look around and figure out how X works"*, the reading itself fills your main context with files you don't need to keep. Better: dispatch a sub-task. The sub-task reads in its own context and reports back a summary. **Your main session stays clean.**

3. **Prefer pointers to embeds.** `@playbook/standards.md` is a one-line reference; the file gets loaded once when needed. Pasting the contents of the playbook into the chat costs context every turn.

4. **Plan in a file, execute in a fresh session.** For larger features, ask the agent to write a `SPEC.md` after a planning conversation. Then start a new session: *"Implement `@SPEC.md`."* The new session has clean context entirely focused on building, and you have a written spec to reference and improve.

5. **Prune persistent instructions.** If the agent keeps doing something despite a rule against it, your instruction file is probably too long. **Cut anything the agent already does correctly without instruction. Less is more.**
...
- **Cursor:** scoped rules in `.cursor/rules/<name>.mdc`
...
1. **Reference-data-as-markdown.** The "rules" of your project — playbook, standards, prompts, business logic — live as markdown files in the repo. The agent reads them as reference on every relevant task. Update the markdown, behaviour changes, no redeployment. **Most powerful pattern in this list.**

2. **Template + structured input → polished output.** A template file (Word, slide deck, HTML) with placeholders, plus a structured input file (markdown, YAML, JSON) describing the variables. The agent fills the template and produces the deliverable. **Far more reliable than asking the agent to "write a document about X" from scratch.**

3. **Diff-based revision.** For revisions, give the agent both the previous version and the new instructions. *"Revise `@drafts/v3.md` according to `@notes/feedback.md`. Show me a diff first; don't apply until I confirm."* Diffs are easier to review than full re-drafts.

4. **Two-pass generation.** First pass: structure and substance. Second pass (often via a sub-agent with a different prompt): tone, polish, consistency, citations. **Trying to do both at once produces mediocre results in both.**

5. **Verified output via second-agent review.** Any output that matters gets a second pass by a different agent (or the same agent in a fresh session) whose only job is to look for problems. The reviewer doesn't share context with the writer, so it catches things the writer was blind to.

6. **Folder-scoped scope guardrail.** One folder per project / matter / customer. The agent operates inside the folder; everything outside is invisible. **This is your privilege boundary, your scope guardrail, and your audit trail.**

7. **Plain-language summary paired with technical output.** Every technical output (memo, report, draft, code change) gets a 3-sentence plain-language summary at the top. For stakeholder communications, that summary is often the only part that gets read. **Build it in by default.**

8. **Risk-graded output.** Group findings by severity rather than by document order. Stakeholders want to see `HIGH` issues first. A consistent rubric (e.g. *blocking / negotiate / accept* or *critical / warning / nit*) means every output speaks the same language.

9. **Human-in-the-loop checkpoints.** Bake review steps into the workflow. A command that runs steps 1–4, then *stops* and asks you to confirm before steps 5–6. **The agent does grunt work; you provide judgement at the right moments.**

10. **Iteratively narrow examples.** When the agent's output keeps drifting from what you want, add more specific examples to the persistent instructions. **One example beats five adjectives; three examples beat one.** Build up your `examples/` folder organically — every time you get a good output, save it.
...
1. **Structural (automatic).** A script or hook checks structure: required sections present, no `[TODO]` markers left in a "final" output, expected output format, no obvious errors. *The agent can write these checks for you.*

2. **Content (semi-automatic).** A separate agent session, with no memory of how the output was produced, reviews the output against the source material. *"Here's the spec. Here's the implementation. Find anything in the implementation that doesn't match the spec, or any spec requirement that's missing."* **Fresh context catches errors the writing session was blind to.**

3. **Human (always).** You read the output. The first two layers exist to make this layer fast — by the time you're reading, structural problems are gone and obvious content problems have been spot-checked. **You're focused on judgement, not proofreading.**
...
- 🏷️ **Tag uncertainty explicitly.** Tell the agent: *"If you're not sure about something, write `[UNCERTAIN: <reason>]` rather than guessing."* Then make `[UNCERTAIN]` part of your structural check.
...
> ⚠️ **The asymmetry to internalise:** **the agent will produce confident-looking wrong answers.** This isn't a bug you can prompt away; it's a property of the model. Your job is to build verification that doesn't rely on you spotting confident-but-wrong by eye.
...
| --- | --- | --- |
| **Kitchen-sink session** | Answers feel less precise; the agent keeps re-explaining things | Clear context, restart with a focused prompt |
| **Over-corrected output** | Each correction makes the output marginally worse, not better | Reset, write a *better* prompt incorporating what you learned, start fresh |
| **Bloated instruction file** | A rule clearly stated in your `CLAUDE.md` / `.cursorrules` / Knowledge tab is being violated | Prune. Move details into skills or scoped rules. Keep only what's broadly applicable |
| **Confident hallucination** | A function name, an API endpoint, a library — fluent and plausible, also wrong | Verification layers from section 14. Treat every unverified specific claim as suspect |
| **Infinite exploration** | Long minutes of file-reading, context window approaching full, no concrete output | Scope tightly. Use sub-agents to keep exploration out of your main context |
...
- **Repo / repository.** A folder containing a project, tracked by `git` so you have full history of every change. *"Repo"* is just shorthand for *"repository"*; same thing.
...
<details>
<summary><strong>🏗️ How software is built: stacks, frameworks, languages</strong></summary>

- **Stack.** The combination of technologies a project uses. *"Next.js + Supabase"* is a stack — meaning Next.js for the frontend and Supabase for the backend and database.
...
<details>
<summary><strong>⚙️ Development tools you'll see in setup instructions</strong></summary>
...
1. Pick one workflow you do every week.
2. Build the smallest possible version.
3. Run it on a real task.
4. Notice what's wrong.
5. Improve it.

By the time you've done this three or four times, you'll have your own intuitions — better than any guide can give you.
